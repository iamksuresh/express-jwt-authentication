...
export class RouteHandlerService implements IRouteHandlerService {
  ...
  handle(
    controller: any,
    handler: (request: Request, response: Response, nextFunction: NextFunction) => any,
  ): any {
    /**
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
      try {
        await this._validateRequestParams(controller, handler, req);
  ...
  private async _validateRequestParams(
    controller: any,
    handler: any,
    request: Request,
  ): Promise<void> {
        ...
        if (controllerBasePath) {
          ...
          if (actions && actions.length > 0) {
            const controllerName =
              this._reflectionService.getMetaData(controllerClass, MetaDataEnum.Name) ||
    ...
    if (actionName) {
      ...
      if (this._reflectionService.isFunction(handler)) {
        ...
        const customActionName =
          this._reflectionService.getMetaData(controller, MetaDataEnum.Name, actionName) ||
          actionName;
        let isPublic = true;
        if (isControllerPublic === true) {
          ...
          isPublic =
            this._reflectionService.getMetaData(controller, MetaDataEnum.IsPublic, actionName) ||
            true;
        }
